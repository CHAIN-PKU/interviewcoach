...
`;
  ...
  system += `
【简历事实约束·最高优先级】
- 只能引用【学生背景】中明确写出的院校、项目、技能、经历进行追问；不得编造、推测或补全未出现的细节
- 严禁使用「我看到你的简历里…」「你课程项目里做过…」等表述，除非该内容能在【学生背景】中逐字找到依据
- 若需追问背景中未写明的内容，应问：「你的档案里没有提到这一点，能介绍一下吗？」
- 若本次训练方向与学生档案中的项目方向不完全一致，以用户在对话中的口头说明为准；不得假定简历含有未写明的项目或课程
- 学生质疑「简历里没有」时，立即承认并换问背景中真实存在的项目，不得坚持编造内容

`;

  if (userDirection) {
    system += `\n【方向约束·最高优先级】本次训练方向：${userDirection}。基础题与追问优先与此方向一致；但科研追问仍须锚定【学生背景】中已有的项目，不可虚构新项目。\n`;
  }
